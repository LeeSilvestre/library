      ...
      >
        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td :class="{'red-text': item.borrow_status === 3, 'green-text': item.borrow_status === 2 }">{{ availableStatus[item.borrow_status] }}</td>
          </tr>
        </template>
      </v-data-table>
    </div>
  ...
  export default {
    ...
    computed: {
      displayedData() {
        ...
        return this.data.filter(item => item.borrow_status == 2 || item.borrow_status == 3);
      },
      formTitle() {
