<template>
      ...
      >
        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td :class="{'red-text': item.borrow_status == 5 || item.borrow_status == 6}">{{ availableStatus[item.borrow_status] }}</td>
            <td>
          ...
          <v-dialog v-model="dialogConfirmation2" max-width="500px">
              <v-card>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue darken-1" @click="backback">Damaged Paid</v-btn>
                  <v-btn color="blue darken-1" @click="backbackback">Lost Paid</v-btn>
  ...
  export default {
    ...
    methods: {
      ...
      backback() {
        axios.put('/borrowstatus/damaged/' + this.selectedBorrowId)
        .then(() => {
          this.loading = true;
          this.loading = false;
          this.closeConfirmation2();
      ...
      backbackback() {
        axios.put('/borrowstatus/lost/' + this.selectedBorrowId)
        .then(() => {
          this.loading = true;
          this.loading = false;
          this.closeConfirmation2();
